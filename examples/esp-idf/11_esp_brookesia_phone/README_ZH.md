...
服务、扬声器系统及未使用的 Boost 库。Actions 还会在 ESP-IDF `v5.5.4` 下额外编译
启用 AI 的配置，避免其条件依赖路径发生静默回归。当前 GMF 0.6 依赖集与 ESP-IDF 6
的组件需求扫描不兼容，因此 IDF 6 通道保持关闭 AI。

本仓库中的 ESP-Brookesia 属于预览集成。Actions 会验证代码可在 ESP-IDF
`v5.5.4` 与 `v6.0.2` 下编译；界面布局、触摸行为、性能及可选服务仍需在
480 × 800 目标开发板上测试。
