/*
 * microSD card read/write demo for the Waveshare ESP32-P4-WIFI6-Touch-LCD-4.3.
 *
...
void setup() {
  ...
  if (!SD_MMC.begin("/sdcard", false /* use the configured 4-bit bus */)) {
  ...
  const char *path = "/hello_lcd43.txt";
  File f = SD_MMC.open(path, FILE_WRITE);
  if (!f) {
    Serial.println("Failed to open file for writing");
    return;
  }
  f.println("Hello from ESP32-P4-WIFI6-Touch-LCD-4.3 microSD demo!");
  f.close();
  Serial.println("Wrote /hello_lcd43.txt");
