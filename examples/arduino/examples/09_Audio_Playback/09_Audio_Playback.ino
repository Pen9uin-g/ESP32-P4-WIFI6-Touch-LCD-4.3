/*
 * ES8311 melody player for the Waveshare ESP32-P4-WIFI6-Touch-LCD-4.3.
 *
...
bool play_note(float freq, uint32_t duration_ms) {
  ...
  while (samples_played < samples) {
    ...
    size_t written = 0;
    const size_t requested = chunk * 2 * sizeof(int16_t);
    const esp_err_t ret = i2s_write(I2S_CH, frame, requested, &written, portMAX_DELAY);
    if (ret != ESP_OK || written != requested) {
      Serial.printf("I2S write failed: %s, bytes: %u/%u\n", esp_err_to_name(ret),
                    (unsigned)written, (unsigned)requested);
      return false;
    }
...
void loop() {
  ...
  for (const Note &note : FUR_ELISE) {
    if (!play_note(note.freq, note.duration_ms)) {
      audio_ready = false;
      digitalWrite(PIN_POWER_AMP, LOW);
      i2s_driver_uninstall(I2S_CH);
      return;
