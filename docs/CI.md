...
`v6.0.2`, producing 24 jobs. A full source-impact run also contains 16 focused
jobs:

- 12 RGB888 jobs for examples 07 through 12 on both ESP-IDF versions;
- 2 AI-enabled Brookesia jobs for example 11;
- 2 USB-minimal jobs with HID touch and UAC audio disabled for example 12.

That makes 40 ESP-IDF build jobs in a complete matrix. The overlays in
